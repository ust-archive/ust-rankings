  ...
  async findContributions(userId: string): Promise<AccountContributions> {
    const [reviews, reactions] = await Promise.all([
      ...
      this.sql<AccountContributions["reactions"]>`
        ...
        WHERE user_id = ${userId}
        UNION ALL
        SELECT 'review'::text, NULL::text, NULL::text, NULL::uuid,
               reaction.review_id, 'emoji'::text, reaction.code,
               reaction.created_at
        ...
        UNION ALL
        SELECT 'review'::text, NULL::text, NULL::text, NULL::uuid,
               vote.review_id, 'thumb'::text, vote.state, vote.updated_at
