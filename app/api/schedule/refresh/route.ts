...
export function createScheduleRefreshHandlers(operation: RefreshOperation) {
  async function handle(request: Request, sha?: string) {
    ...
    try {
      ...
      const { InvalidScheduleQueryError, ScheduleRefreshError } = await import(
        "@/lib/schedule/server"
      );
      if (error instanceof InvalidScheduleQueryError)
        return Response.json({ error: error.message }, { status: 400 });
      if (error instanceof ScheduleRefreshError)
...
async function productionRefresh(options: { sha?: string }) {
  ...
  return refreshSchedule(options, productionScheduleRefreshDependencies());
}

const productionHandlers = createScheduleRefreshHandlers(productionRefresh);
