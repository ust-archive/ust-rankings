...
function groupBy<I, O>(
  key: (it: I) => O,
): (r: Record<string, I[]>, v: I, i: number, a: I[]) => Record<string, I[]> {
...
export function InstructorRatingChart({ teachRatings, thumbRatings }: Props) {
  const teachRatingGroups = teachRatings.reduce(
    groupBy((it) => it.semester),
    {},
  );
  const thumbRatingGroups = thumbRatings.reduce(
    groupBy((it) => it.semester),
    {},
  );
  const maxSemester = Math.max(
    ...teachRatings.map((it) => it.semester),
    ...thumbRatings.map((it) => it.semester),
  );
  ...
  let semesters = Array.from(
    { length: maxSemester - minSemester + 1 },
    (_, i) => i + minSemester,
  ).sort((a, b) => b - a);
  ...
  const chartData = semesters.map((semester) => ({
    ...
    "Rating (Thumbs Up)": mean(
      (thumbRatingGroups[semester] ?? []).map((it) => it.rating),
    ),
    "Rating (Teaching)": mean(
      (teachRatingGroups[semester] ?? []).map((it) => it.rating),
    ),
    Samples:
      ((teachRatingGroups[semester]?.length ?? 0) +
        (thumbRatingGroups[semester]?.length ?? 0)) /
      2,
  }));
