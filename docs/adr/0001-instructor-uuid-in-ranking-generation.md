...
Identity is published in four Parquet relations in that Ranking Generation: one current row per Instructor UUID, Instructor Aliases, flattened identity events, and explicit Course scopes for splits and Instructor Association Calibrations. Instructor ratings use Instructor UUID as their identity grain; Canonical Instructor Name remains display data.
...
Merge, split, ITSC, and Instructor Association Calibration corrections are idempotent, version-controlled pipeline input. A calibration reassigns one source name within a Course or Course Offering to an existing Instructor UUID; it does not merge identities. Two Instructors may share one Canonical Instructor Name: split history and its affected Course Offering associations keep their evidence on distinct Instructor UUIDs, while an unqualified same-name collision fails publication. Complete Schedule history carries resolved Course Offerings forward but does not justify guessing a new same-name association in another Term. The app does not overlay a registry at refresh. Schedule resolves a Class source name against the accepted ranking generation at read time; it does not snapshot Instructor UUIDs into a Schedule generation.
...
## Typed Instructor Identity History projection

The four published identity relations remain the Ranking Generation contract. Their fourth relation keeps the historical filename `instructor-split-affected-associations.parquet`, but its rows are typed Instructor Association Corrections with `correction_type` (`split` or `calibration`) and `target_uuid`. The pure Instructor Identity History module validates and projects merge redirects, ITSC history, and scoped correction matching once; pipeline and runtime consume that projection. Accepted `course-instructors.parquet` UUIDs remain authoritative, while raw Schedule and Review associations apply Calibrations and fail closed on mismatched split corrections.

A `master` publication runs verified CI, publishes the rebuilt Ranking Generation, then deploys and activates that exact generation. Scheduled and manual data publications continue refreshing the already-compatible runtime directly.
